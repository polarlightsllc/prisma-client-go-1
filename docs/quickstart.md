...
2) Get Prisma Client Go
    ...
    ```shell script
    go get github.com/steebchen/prisma-client-go-1
    ```
...
3) Prepare your database schema in a `schema.prisma` file. For example, a simple schema with a sqlite database and
    ...
    go run github.com/steebchen/prisma-client-go-1 migrate dev --name init
    ```

    After the migration, the Prisma Client Go client is automatically generated in your project.

    If you just want to re-generate the client, run `go run github.com/steebchen/prisma-client-go-1 generate`.
